...
# onclick 이벤트 핸들러는 함수를 호출하는 것이 아니라 함수를 전달하는 것이다.

1. 함수 전달 vs 함수 호출:

   - 올바른 방법: onClick={handleClick}
   - 잘못된 방법: onClick={handleClick()}

2. 이유:

   - React는 이벤트가 발생했을 때 실행할 함수를 필요로 한다.
   - 함수를 호출하면 (괄호를 붙이면) 그 즉시 함수가 실행되고, 그 결과가 onClick에 할당된다.
   - 이는 의도하지 않은 동작을 유발할 수 있다.

3. 함수 참조 전달의 장점:

   - 이벤트 발생 시에만 함수가 실행된다.
   - 불필요한 함수 호출을 방지하여 성능을 개선한다.
   - React의 이벤트 시스템과 조화롭게 작동한다.

4. 매개변수가 필요한 경우:

   - 화살표 함수를 사용하여 래핑할 수 있다:
     onClick={() => handleClick(param)}

5. 주의사항:
   - 인라인 함수 사용 시 렌더링마다 새 함수가 생성될 수 있으므로, 성능에 주의해야 한다.
   - 가능하면 컴포넌트 내부에서 함수를 정의하고 참조를 전달하는 것이 좋다.
